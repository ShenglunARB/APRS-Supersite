...
def find_daily_raw_datafile(folder, date):
    ...
    folderpath = folder + '/Level1A_Raw_Data/' + YY
...
def clean_warning_data(df_daily):
    ...
    for p in parameter:
        df_daily_nowarning.loc[df_daily_nowarning[p] < 50, p] = np.nan
...
def processed_data_folder_level2b(folder):
    """
    Generate folder path to store processed data.
    Input:
        folder: str, folder path to the analyzer data. From function: find_data_folder
    ...
    folder_level2b = folder + '/Level2B_Processed_Data_1hr'
    if not os.path.exists(folder):
        os.makedirs(folder)

    return folder_level2b
    ...
    if filepath is not None:
        ...
        if df_daily_nowarning.empty:
            ...
            if average_time == '1min':
                ...
                store_processed_data(folder_level2a, site, species, date, df_daily_clean)
            
            # process data to 1hr resolution
            elif average_time == '60min':
                df_avg = average_daily_data(df_daily_clean, average_time)
                folder_level2b = processed_data_folder_level2b(folder)
                store_processed_data(folder_level2b, site, species, date, df_avg)
...
if __name__ == "__main__":
    ...
    dates = pd.date_range(start='2024-10-12', end='2024-11-21', freq='D')
